	...
	$('#btn-auth-renewal').on('click', function(e){
		...
		$.ajax({
			...
			success : function(s) {
				...
				new PNotify({
					title: 'Success',
					text: 'Renewal Authorised in Batch Successfully...This is a background job and will take sometime',
					type: 'success',
					styling: 'bootstrap3'
				});
...
function makeReadyRenewal(){
	$('#btn-make-ready').on('click', function(e){
		...
		if(arr.length==0){
			bootbox.alert("No Record Selected to Make Ready..");
			return;
		}
		$('#myPleaseWait').modal({
			backdrop: 'static',
        ...
        order: [[ 1, 'asc' ]],
			"columns": [
				...
				{ 
					"data": "policyId",
					"render": function ( data, type, full, meta ) {
						return '<form action="editrenewals" method="post"><input type="hidden" name="id" value='+full.policyId+'><input type="submit"  class="btn btn-success btn btn-info btn-xs" value="Edit" ></form>';
					 }
