...
public interface ChequeTransDtlsRepo extends PagingAndSortingRepository<ChequeTransDtls, Long>, QueryDslPredicateExecutor<ChequeTransDtls> {
    ...
    @Query(value = "select ctd_no,ctd_dc,ctd_narrative,ctd_amount,acc.co_code,acc.co_name,sbb.ob_name,total_rows=COUNT(*) OVER()  from sys_chq_trans_dtls \n" +
            ...
            "order by ctd_no desc OFFSET :pageNo*:limit ROWS FETCH NEXT :limit ROWS ONLY",nativeQuery = true)
