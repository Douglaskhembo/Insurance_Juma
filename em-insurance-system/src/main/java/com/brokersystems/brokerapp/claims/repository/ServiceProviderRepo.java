...
public interface ServiceProviderRepo extends PagingAndSortingRepository<ServiceProviderDef, Long>, QueryDslPredicateExecutor<ServiceProviderDef> {
    ...
    @Query(value = "select provd_id,provd_email,provd_name,provd_mobile,a.created_date,provd_type_id,sbu.user_username,total_rows=COUNT(*) OVER() from sys_brk_serv_providers a  \n" +
            " join sys_brk_users sbu on sbu.user_id =a.created_user where provd_type_id = :typeId and  (lower(provd_name) like :search)\n" +
            "order by provd_name asc OFFSET :pageNo*:limit ROWS FETCH NEXT :limit ROWS ONLY",nativeQuery = true)
    ...
    @Query(value = "select provd_id,provd_name,total_rows=COUNT(*) OVER() from sys_brk_serv_providers  \n" +
